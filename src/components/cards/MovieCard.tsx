...
export function MovieCard({ movie, type = "movie", index = 0 }: { movie: TMDBMovie; type?: "movie" | "tv"; index?: number }) {
  ...
  return (
    ...
    >
      <Link
        href={href}
        className="group relative block overflow-hidden rounded-xl transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl"
      >
        ...
        <div className="absolute inset-0 rounded-xl ring-1 ring-inset ring-white/0 transition duration-300 group-hover:ring-white/10" />
      </Link>
      <div className="mt-2">
...
export function HeroCard({ movie, type = "movie", index = 0 }: { movie: TMDBMovie; type?: "movie" | "tv"; index?: number }) {
  const title = movie.title || movie.name || "Untitled";
  const href = type === "tv" ? `/tv/${movie.id}` : `/title/${movie.id}`;
  ...
  return (
    ...
    >
      <Link
        href={href}
        className="group relative block h-full overflow-hidden rounded-xl border border-white/[0.07] transition-all duration-300 hover:border-amber-500/40 hover:shadow-[0_20px_60px_-15px_rgba(245,158,11,0.25)]"
      >
        <div
